...
const LOG_TARGET: &str = "provider-rpc";

pub struct ProviderRpcConfig<T, FL, FS> {
    pub file_storage: Arc<RwLock<FL>>,
    pub forest_storage: Arc<RwLock<FS>>,
    _marker: PhantomData<T>,
}

impl<T, FL, FS> Clone for ProviderRpcConfig<T, FL, FS> {
    fn clone(&self) -> Self {
        Self {
...
impl<T, FL, FS> ProviderRpcConfig<T, FL, FS>
where
...
pub struct ProviderRpc<T, FL, FS> {
    file_storage: Arc<RwLock<FL>>,
    forest_storage: Arc<RwLock<FS>>,
    _marker: PhantomData<T>,
}

impl<T, FL, FS> ProviderRpc<T, FL, FS>
where
    T: TrieLayout + Send + Sync + 'static,
    FL: FileStorage<T> + Send + Sync,
    FS: ForestStorage<T> + Send + Sync,
{
    pub fn new(provider_rpc_config: ProviderRpcConfig<T, FL, FS>) -> Self {
        Self {
            file_storage: provider_rpc_config.file_storage,
            forest_storage: provider_rpc_config.forest_storage,
            _marker: Default::default(),
        }
    }
}

/// Interface generated by the `rpc` macro from our `ProviderApi` trait.
...
#[async_trait]
impl<T, FL, FS> ProviderApiServer for ProviderRpc<T, FL, FS>
where
