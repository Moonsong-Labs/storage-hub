...
impl FtpFileHandler {
    ...
    fn parse_url(url: &Url) -> Result<(String, u16, Option<String>, Option<String>, String), RemoteFileError> {
    ...
    fn ftp_error_to_remote_error(error: FtpError) -> RemoteFileError {
        match error {
            FtpError::UnexpectedResponse(ref resp) => {
                match resp.status {
...
impl RemoteFileHandler for FtpFileHandler {
    ...
    async fn download_chunk(
        ...
        while total_read < length as usize {
            ...
            let n = reader.read(&mut buffer[total_read..total_read + to_read]).await
