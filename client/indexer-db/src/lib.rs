...
const LOG_TARGET: &str = "shc-indexer-db";
...
pub async fn setup_db_pool(database_url: String) -> Result<DbPool, DbSetupError> {
    // Default ON; set SH_INDEXER_DB_AUTO_MIGRATE to "false"/"0"/"off" to disable
    let should_auto_migrate = std::env::var("SH_INDEXER_DB_AUTO_MIGRATE")
        .map(|v| {
    ...
    if should_auto_migrate {
        info!(target: LOG_TARGET, "👨‍💻 Running indexer database migrations...");
        ...
        migrate_result?;
        info!(target: LOG_TARGET, "👨‍💻 Indexer database migrations completed");
    }
