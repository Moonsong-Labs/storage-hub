    ...
    RuntimeApi::RuntimeApi: StorageEnableApiCollection,
{
    async fn handle_event(&mut self, event: ProcessFileDeletionRequest) -> anyhow::Result<()> {
        ...
        let bsp_ids = shc_indexer_db::models::bsp::BspFile::get_bsps_for_file_key(
            &mut conn,
            file_key.as_ref(),
        )
        .await
        ...
        let bucket_target = shc_fisherman_service::events::FileDeletionTarget::BucketId(
            H256::from(bucket_id_array),
        );
        ...
        let file_account_ref = &file.account;
    ...
    async fn process_deletion_for_target(
        ...
        let forest_proof = {
            ...
            let indexer_db_pool = self
                ...
                .as_ref()
                .ok_or_else(|| {
                    anyhow!("Indexer is disabled but a file deletion event was received")
                })?;
        ...
        let file_owner =
            AccountId32::new(file_account.try_into().expect("Length already validated"));
