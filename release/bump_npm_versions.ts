...
/**
 * Bump npm/package.json versions for a given family so that:
 *   package_version >= target_version (semver, x.y.z)
 *
 * - Reads configuration from release/versions.json
 * - Discovers package.json files under the configured roots
 * - Updates only the "version" field (not dependency ranges)
 *
 * Usage examples:
...
function printHelp(): void {
  console.log(
    [
      ...
      "",
      "Options:",
      "  --family <name>    Version family to use from release/versions.json (e.g. sdk, types_bundle, api_augment)",
      "  --version <x.y.z>  Override target version for the chosen family",
      "  --dry-run          Show planned changes without modifying files",
      "  --help             Show this help message",
      "",
      "Behaviour:",
      '  - Only updates the top-level "version" field in package.json files.',
      "  - Skips packages whose version is already greater than or equal to the target."
    ].join("\n")
  );
}
...
function main(): void {
  ...
  // If no family is specified, we run for all known npm-related families.
  const defaultNpmFamilies = ["sdk", "types_bundle", "api_augment"];
  const familiesToRun =
    args.family !== null
      ? [args.family]
      : defaultNpmFamilies.filter((name) => allFamilies.includes(name));

  if (familiesToRun.length === 0) {
    console.warn("No npm-related families found in manifest to operate on.");
    return;
  }
  ...
  console.log("==============================================");
  console.log(" npm package version bump");
  ...
  for (const familyName of familiesToRun) {
    ...
    if (packageFiles.length === 0) {
      console.warn(
        `No package.json files found for the configured roots of family '${familyName}'.`
      );
      // Continue to the next family instead of returning.
      // eslint-disable-next-line no-continue
      continue;
    }
