...
export class HttpClient {
    ...
    async request<T>(method: 'GET' | 'POST' | 'PUT' | 'DELETE', path: string, options: RequestOptions = {}): Promise<T | Response> {
        ...
        try {
            ...
            if (options.raw) {
                if (!res.ok) {
                    // For errors, we need to consume the body to get error details
