...
function decodeBase64ToBytes(b64: string): Uint8Array {
	...
	if (
		...
		typeof (
			Buffer as unknown as { from?: (s: string, enc: "base64") => Uint8Array }
		).from === "function"
	) {
		const buf = (
			Buffer as unknown as { from: (s: string, enc: "base64") => Uint8Array }
...
export async function initWasm(): Promise<void> {
	...
	initPromise = (async () => {
		...
		const b64 =
			typeof mod.WASM_BASE64 === "string"
				? (mod.WASM_BASE64 as string)
				: undefined;
		if (!b64 || b64.length === 0) {
			throw new Error(
				"Embedded WASM is missing or empty. Ensure build generated _wasm_embed.ts.",
			);
		}
