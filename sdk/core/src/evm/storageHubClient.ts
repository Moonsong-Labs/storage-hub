...
export class StorageHubClient {
	...
	private async estimateGas(
		...
		const multiplier =
			options?.gasMultiplier ?? StorageHubClient.DEFAULT_GAS_MULTIPLIER;
	...
	private buildTxOptions(
		...
		if (useEip1559) {
			...
			if (options?.maxPriorityFeePerGas)
				txOpts.maxPriorityFeePerGas = options.maxPriorityFeePerGas;
		} else {
	...
	private validateStringLength(
		...
		if (bytes.length > maxBytes) {
			throw new Error(
				`${label} exceeds maximum length of ${maxBytes} bytes (got ${bytes.length})`,
			);
		}
	...
	async createAndAssociateCollectionWithBucket(
		...
		const contract = this.getWriteContract();
		return await contract.write.createAndAssociateCollectionWithBucket?.(
			args,
			txOpts,
		);
	}
	...
	async issueStorageRequest(
		...
		const peerIdsHex = peerIds.map((peerId, i) =>
			this.validateStringLength(
				peerId,
				StorageHubClient.MAX_PEER_ID_BYTES,
				`Peer ID ${i + 1}`,
			),
		);
		const args = [
