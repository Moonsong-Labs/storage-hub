...
export function FileManager({ walletClient, publicClient, walletAddress, mspClient, storageHubClient }: FileManagerProps) {
  ...
  const loadBuckets = async () => {
    ...
    try {
      ...
      let bucketList: any[] = [];
      try {
        bucketList = await mspClient.buckets.list();
      } catch (sdkError: any) {
  ...
  const downloadFile = async (file: FileTree) => {
    ...
    try {
      ...
      console.log('📥 Calling mspClient.files.downloadByKey...');
      const downloadResult = await mspClient.files.downloadByKey(fileKey);
