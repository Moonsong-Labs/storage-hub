...
export class FilesModule extends ModuleBase {
	...
	getFiles(
		...
		return this.ctx.http.get<FileListResponse>(path, {
			...
			...(options?.signal ? { signal: options.signal } : {}),
			...(options?.path
				? { query: { path: options.path.replace(/^\/+/, "") } }
				: {}),
		});
	}
	...
	async uploadFile(
		...
		if (
			...
			!computedFileKey.every(
				(byte, index) => byte === expectedFileKeyBytes[index],
			)
		) {
			throw new Error(
	...
	async downloadByKey(
		...
		const contentLength =
			typeof parsedLength === "number" && Number.isFinite(parsedLength)
				? parsedLength
				: null;
	...
	private async coerceToFormPart(
		...
		if (file instanceof Uint8Array)
			return new Blob([file.buffer as ArrayBuffer]);
