...
fn commitment_verifier_challenge_exactly_first_key_success() {
    ...
    // Verify proof
    let proof_keys =
        ForestVerifier::<LayoutV1<RefHasher>>::verify_proof(&root, &[*challenge_key], &proof)
...
fn commitment_verifier_multiple_exact_challenge_keys_success() {
    ...
    // Verify proof
    let proof_keys =
        ForestVerifier::<LayoutV1<RefHasher>>::verify_proof(&root, &challenge_keys, &proof)
            .expect("Failed to verify proof");
...
fn commitment_verifier_empty_proof_and_root_failure() {
    ...
    assert_eq!(
        ForestVerifier::<LayoutV1<RefHasher>>::verify_proof(
            &empty_root,
            &[*challenge_key],
            &empty_proof
        ),
...
fn commitment_verifier_no_challenges_failure() {
    ...
    assert_eq!(
        ForestVerifier::<LayoutV1<RefHasher>>::verify_proof(&root, &[], &proof),
