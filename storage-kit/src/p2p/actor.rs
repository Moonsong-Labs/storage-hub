...
impl ActorEventLoop<P2PModule> for P2PEventLoop {
    ...
    async fn run(&mut self) {
        info!(
            "P2PModule starting up with peerId {:?}",
            self.actor.swarm.local_peer_id()
        );
        loop {
            select! {
...
pub enum P2PModuleCommand {
    ...
    Multiaddresses {
        channel: oneshot::Sender<Vec<Multiaddr>>,
    },
}
    ...
    async fn handle_message(&mut self, command: Self::Message) {
        match command {
            P2PModuleCommand::ExternalDial { multiaddr, channel } => {
                ...
                channel
                    .send(Ok(()))
                    .map_err(|_| anyhow!("Failed to send dial command"))
                    .unwrap();
            }
            P2PModuleCommand::Multiaddresses { channel } => {
                ...
                channel
                    .send(multiaddresses)
                    .map_err(|_| anyhow!("Failed to send multiaddresses"))
                    .unwrap();
            }
        }
    }
