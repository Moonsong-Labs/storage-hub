use crate::{
    mock::*, types::*, BoundedQueue, DeadlineTickToProviders, Error, Event,
    FirstSubmittersProviders, PendingCommitments, QueueParameters, RandomSeedMixer,
...
mod add_randomness {
    ...
    fn revealed_randomness_gets_mixed_with_existent_randomness() {
        ...
        // And the Provider should have as deadline this new deadline
        assert!(DeadlineTickToProviders::<Test>::get(third_deadline).contains(&alice_provider_id));
...
mod slashing {
    ...
    fn providers_that_miss_deadlines_get_stored_to_be_marked_as_slashable() {
        ExtBuilder::build().execute_with(|| {
            ...
            let alice_deadline = FirstSubmittersProviders::<Test>::get(alice_provider_id).unwrap();
            let bob_deadline = FirstSubmittersProviders::<Test>::get(bob_provider_id).unwrap();
            let charlie_deadline =
                FirstSubmittersProviders::<Test>::get(charlie_provider_id).unwrap();
            ...
            // They also should have their next deadline to be one tolerance period after this one
            let tolerance_period: BlockNumberFor<Test> =
                <<Test as crate::Config>::MaxSeedTolerance as Get<u32>>::get().into();
