...
mod fixed_rate_streams {
    ...
    mod update_last_chargeable_block {
        ...
        #[test]
        fn update_last_chargeable_block_fails_if_stream_does_not_exist() {
            ExtBuilder::build().execute_with(|| {
                let alice: AccountId = 0;
                let bob: AccountId = 1;
        ...
        #[test]
        fn update_last_chargeable_block_fails_if_bsp_account_has_not_registered_as_bsp() {
            ExtBuilder::build().execute_with(|| {
                let bob: AccountId = 1;
...
mod dynamic_rate_streams {
    ...
    mod create_stream {
        ...
        fn create_payment_stream_works() {
            ExtBuilder::build().execute_with(|| {
                ...
                // Get the payment stream information
                let payment_stream_info =
                    PaymentStreams::get_dynamic_rate_payment_stream_info(&alice_msp_id, &bob)
                        .unwrap();
        ...
        fn create_payment_stream_fails_if_user_is_flagged_as_without_funds() {
            ExtBuilder::build().execute_with(|| {
                ...
                run_to_block(System::block_number() + 10);
                assert_ok!(
                    <PaymentStreams as PaymentManager>::update_chargeable_price_index(
                        &alice_msp_id,
                        &bob,
                        current_price_index
                    )
        ...
        fn create_payment_stream_fails_if_user_has_too_many_streams() {
            ExtBuilder::build().execute_with(|| {
                ...
                // Check that the payment stream from Bob to Charlie was not created
                assert!(matches!(
                    PaymentStreams::get_dynamic_rate_payment_stream_info(&charlie_msp_id, &bob),
                    Err(Error::<Test>::PaymentStreamNotFound)
                ));
    ...
    mod update_stream {
        ...
        fn updated_payment_stream_charges_pending_blocks_with_old_rate() {
            ExtBuilder::build().execute_with(|| {
                ...
                assert_eq!(
                    NativeBalance::free_balance(&bob),
                    bob_balance_updated_deposit - 10 * current_price
                );
                System::assert_has_event(
                    Event::<Test>::PaymentStreamCharged {
                        user_account: bob,
    ...
    mod delete_stream {
        ...
        fn delete_payment_stream_charges_pending_blocks() {
            ExtBuilder::build().execute_with(|| {
                ...
                assert_eq!(
                    NativeBalance::free_balance(&bob),
                    bob_new_balance + deposit_amount - current_price * 10
                );
                System::assert_has_event(
                    Event::<Test>::PaymentStreamCharged {
                        user_account: bob,
                ...
                assert_eq!(
                    NativeBalance::free_balance(&bob),
                    bob_initial_balance - 10 * current_price
                );
            });
        }
    }
    ...
    mod charge_stream {
        ...
        fn charge_payment_streams_works() {
            ExtBuilder::build().execute_with(|| {
                ...
                assert_eq!(
                    NativeBalance::free_balance(&bob),
                    bob_new_balance - 10 * current_price
                );
                System::assert_has_event(
                    Event::<Test>::PaymentStreamCharged {
                        user_account: bob,
        ...
        fn charge_payment_works_after_two_charges() {
            ExtBuilder::build().execute_with(|| {
                ...
                assert_eq!(
                    NativeBalance::free_balance(&bob),
                    bob_new_balance - 10 * current_price - 20 * current_price
                );
                System::assert_has_event(
                    Event::<Test>::PaymentStreamCharged {
                        user_account: bob,
                        provider_id: alice_msp_id,
                        amount: 20 * current_price,
                    }
                    .into(),
                );
        ...
        fn charge_payment_streams_fails_if_total_amount_would_overflow_balance_type() {
            ExtBuilder::build().execute_with(|| {
                ...
                let deposit_amount = current_price
                    * (amount_provided as u128)
                    * new_stream_deposit_blocks_balance_typed;
                let bob_balance_after_deposit = bob_new_balance - deposit_amount;
                assert_eq!(NativeBalance::free_balance(&bob), bob_balance_after_deposit);

                // Set the last valid proof of the payment stream from Bob to Alice to 1000 blocks ahead
                let current_price_index = current_price_index + 1000 * current_price;
                run_to_block(System::block_number() + 1000);
                assert_ok!(
                    <PaymentStreams as PaymentManager>::update_chargeable_price_index(
                        &alice_msp_id,
        ...
        fn charge_payment_streams_unflags_user_if_it_now_has_enough_funds() {
            ExtBuilder::build().execute_with(|| {
                ...
                // Deposit enough funds to Bob's account
                let deposit_amount = current_price * 10;
                assert_ok!(NativeBalance::mint_into(&bob, deposit_amount));
                ...
                assert_eq!(
                    NativeBalance::free_balance(&bob),
                    bob_new_balance + deposit_amount - 10 * current_price
                );
                System::assert_last_event(
                    Event::<Test>::PaymentStreamCharged {
                        user_account: bob,
    ...
    mod update_chargeable_price_index {
        ...
        #[test]
        fn update_chargeable_price_index_fails_if_stream_does_not_exist() {
            ExtBuilder::build().execute_with(|| {
                let alice: AccountId = 0;
                let bob: AccountId = 1;
                let amount_provided = 100;
        ...
        #[test]
        fn update_chargeable_price_index_fails_if_msp_account_has_not_registered_as_msp() {
            ExtBuilder::build().execute_with(|| {
                let bob: AccountId = 1;
        ...
        #[test]
        fn update_chargeable_price_index_fails_if_trying_to_set_greater_than_current_block_number()
        {
            ExtBuilder::build().execute_with(|| {
                let alice: AccountId = 0;
                let bob: AccountId = 1;
