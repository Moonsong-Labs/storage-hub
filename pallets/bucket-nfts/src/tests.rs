...
mod burn_tests {
    ...
    fn burn_success() {
        new_test_ext().execute_with(|| {
            ...
            let issuer_origin = RuntimeOrigin::signed(issuer.clone());
            let recipient = Keyring::Bob.to_account_id();
            let recipient_origin = RuntimeOrigin::signed(recipient.clone());
            let msp = Keyring::Charlie.to_account_id();
            let bucket_name = BoundedVec::try_from(b"bucket".to_vec()).unwrap();
            ...
            // Dispatch a signed extrinsic.
            assert_ok!(BucketNfts::burn(recipient_origin, bucket_id, 999));

            // Assert that the item does not exist in the collection.
            assert!(!pallet_nfts::pallet::Account::<Test>::contains_key((
                recipient.clone(),
                0,
                999
            ...
            // Assert that the item metadata does not exist.
            assert!(!pallet_nfts::pallet::ItemMetadataOf::<Test>::contains_key(
                0, 999
            ));
        });
    ...
    #[test]
    fn burn_bucket_not_found_fail() {
        new_test_ext().execute_with(|| {
            let issuer = Keyring::Alice.to_account_id();
            let user_origin = RuntimeOrigin::signed(Keyring::Bob.to_account_id());
            let bucket_name = BoundedVec::try_from(b"bucket".to_vec()).unwrap();
