...
pub type OverwriteFn<ELEMENT> = fn(element: &mut ELEMENT) -> (bool, Weight);
...
impl<W, ELEMENT, SIZE, Q, P> BoundedQueue<W, ELEMENT, SIZE, Q, P> where ELEMENT: FullCodec + Clone, Q: StorageValue<BoundedVec<ELEMENT, SIZE>, Query=BoundedVec<ELEMENT, SIZE>>, P: StorageValue<(u32, u32), Query=(u32, u32)>, SIZE: Get<u32>, W: Get<RuntimeDbWeight> {
    fn convert_logical_index_to_actual(logical_index: u32, queue_length: u32) -> Result<usize, QueueError> {
    ...
    pub fn overwrite_queue(map_fn: &dyn Fn(&mut ELEMENT) -> (bool, Weight), mut start_index: usize) -> Result<Weight, QueueError> {
        let mut queue = Q::get();
        let mut total_map_weight = Weight::zero();

        let initial_actual_index = Self::convert_logical_index_to_actual(start_index as u32, queue.len() as u32)?;
        let (mut should_continue, mut current_map_weight) = map_fn(&mut queue[initial_actual_index]);
        total_map_weight += current_map_weight;

        while should_continue {
            start_index += 1;
            if let Ok(actual_index) = Self::convert_logical_index_to_actual(start_index as u32, queue.len() as u32) {
    ...
    pub fn shift_queue() -> Weight {
        ...
        let new_head = if head + 1 > size {
            0
        } else {
            head + 1
        };

        let new_tail = if tail + 1 > size {
            0
        } else {
            tail + 1
        };
