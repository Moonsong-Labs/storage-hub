...
describeBspNet(
  ...
  ({ before, createBspApi, it, createUserApi }) => {
    ...
    it("bsp volunteers when issueStorageRequest sent", async () => {
      ...
      strictEqual(
        bspConfirmRes_bspId.toHuman(),
        userApi.shConsts.TEST_ARTEFACTS[source].fingerprint
      );

      await sleep(1000); // wait for the bsp to process the BspConfirmedStoring event
      const bspForestRootAfterConfirm =
        await bspApi.rpc.storagehubclient.getForestRoot(null);
      strictEqual(
        bspForestRootAfterConfirm.toString(),
        bspConfirmRes_newRoot.toString()
      );
      notEqual(
        bspForestRootAfterConfirm.toString(),
        initialBspForestRoot.toString()
      ...
      await it("downloaded file passed integrity checks", async () => {
        ...
        strictEqual(
          sha,
          userApi.shConsts.TEST_ARTEFACTS["res/whatsup.jpg"].checksum
        );
      });
    });
  }
  ...
  ({ before, createBspApi, createUserApi, it }) => {
    ...
    it("bsp volunteers multiple files properly", async () => {
      ...
      const fileKeys = storageRequestEvents.map((event) => {
        const dataBlob =
          userApi.events.fileSystem.NewStorageRequest.is(event.event) &&
          event.event.data;
        if (!dataBlob) {
          throw new Error("Event doesn't match Type");
        }
      ...
      const bspForestRootAfterConfirm2 =
        await bspApi.rpc.storagehubclient.getForestRoot(null);
      strictEqual(
        bspForestRootAfterConfirm2.toString(),
        bspConfirm2Res_newRoot.toString()
      );
    });
  }
);
...
describeBspNet(
  ...
  ({ before, createBspApi, createUserApi, it }) => {
    ...
    before(async () => {
      ...
      const { rpcPort } = await userApi.docker.onboardBsp({
        bspSigner: bspTwoKey,
        name: "sh-bsp-two",
    ...
    it("bsp volunteers multiple files properly", { only: true }, async () => {
      // Replicate to 1 BSPs to simulate a BSP confirming a fulfilled storage request, 1 block to maxthreshold (i.e. instant acceptance)
      await userApi.sealBlock(
      ...
      // Wait for 3 volunteering requests from each BSP.
      userApi.wait.bspVolunteerInTxPool(source.length * 2);

      // Seal block
      await userApi.sealBlock();

      // Wait for the BSP to confirm
      await sleep(500);
      ...
      const matchedEvents = await userApi.assert.eventMany(
        "fileSystem",
        "BspConfirmedStoring"
      );

      const bspConfirmRes = matchedEvents.map((event) => {
        const dataBlob =
          userApi.events.fileSystem.BspConfirmedStoring.is(event.event) &&
          event.event.data;
        if (!dataBlob) {
          throw new Error("Event doesn't match Type");
        }
      ...
      const bspConfirmEvent = bspConfirmRes.find(
        (event) => event.bspId.toString() === userApi.shConsts.DUMMY_BSP_ID
      );
      const bspForestRootAfterConfirm =
        await bspApi.rpc.storagehubclient.getForestRoot(null);
      strictEqual(
        bspForestRootAfterConfirm.toString(),
        bspConfirmEvent?.newRoot.toString()
      );

      const bspTwoConfirmEvent = bspConfirmRes.find(
        (event) => event.bspId.toString() === ShConsts.BSP_TWO_ID
      );
      const bspTwoForestRootAfterConfirm =
        await bspTwoApi.rpc.storagehubclient.getForestRoot(null);
      strictEqual(
        bspTwoForestRootAfterConfirm.toString(),
        bspTwoConfirmEvent?.newRoot.toString()
      );
