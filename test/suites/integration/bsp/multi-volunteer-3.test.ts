...
await describeBspNet(
  "BSPNet: Mulitple BSP Volunteering - 3",
  ({ before, it, createUserApi, createApi }) => {
    ...
    it("multiple BSPs volunteer to multiple requests", async () => {
      ...
      const { rpcPort: bspTwoRpcPort } = await api.docker.onboardBsp({
        bspSigner: bspTwoKey,
        name: "sh-bsp-two",
      ...
      const { rpcPort: bspThreeRpcPort } = await api.docker.onboardBsp({
      ...
      // Create API connections and wait for newly onboarded BSPs to catch up
      // to chain tip. During onboarding, several blocks were sealed that the
      // BSPs may not have imported yet, causing them to miss storage request events.
      const bspTwoApi = await createApi(`ws://127.0.0.1:${bspTwoRpcPort}`);
      const bspThreeApi = await createApi(`ws://127.0.0.1:${bspThreeRpcPort}`);
