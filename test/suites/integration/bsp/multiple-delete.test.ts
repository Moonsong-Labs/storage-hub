...
describeBspNet("Single BSP Volunteering", ({ before, createBspApi, it, createUserApi }) => {
    ...
    it("Volunteer for multiple files and delete them (failing to batch when confirming)", { skip: "cannot store files again after they have been deleted once" }, async () => {
        const source = ["res/whatsup.jpg", "res/adolphus.jpg", "res/cloud.jpg"];
        const destination = ["test/whatsup.jpg", "test/adolphus.jpg", "test/cloud.jpg"];
        ...
        assert(confirmStopStoringEvents.length == 1, "two of the extrinsincs should fail because of wrong inclusion proof");
    });
});