...
describeBspNet(
  ...
  ({ before, it, createUserApi }) => {
    ...
    it(
      "Change capacity ext called before volunteering for file size greater than available capacity",
      { only: true },
      async () => {
        ...
        assertEventPresent(api, "providers", "CapacityChanged", await api.query.system.events());
