...
await describeMspNet(
  ...
  ({ before, it, createUserApi, createMsp1Api, createSqlClient, createIndexerApi }) => {
    ...
    it(
      "Should encrypt and upload encrypted variant (signature)",
      async () => {
    ...
    it(
      "Should encrypt and upload encrypted variant (password)",
      async () => {
        if (!encryptedOriginalBytes) {
          const originalPath = fileURLToPath(
            new URL("../../../../docker/resource/adolphus.jpg", import.meta.url)
    ...
    it(
      "Should download and decrypt encrypted variant (signature)",
      async () => {
        ...
        await decryptFile({
          ...
          getIkm: async (hdr) => {
            ...
            assert(hdr.challenge?.length === 32, "Encrypted(signature) header should include challenge");
    ...
    it(
      "Should download and decrypt encrypted variant (password)",
      async () => {
        assert(encryptedOriginalBytes, "encryption/upload tests must run first");
